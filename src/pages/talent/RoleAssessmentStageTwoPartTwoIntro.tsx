...
/** Part 3 · Reasoning — content from GET .../gates/2/pillars/reasoning/intro */
const RoleAssessmentStageTwoPartTwoIntro: React.FC = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useStage2PillarIntroQuery(activeAssessmentId || '', 'reasoning');
  ...
  const beginPartLabel = pillarIntro.ctas?.beginPartLabel || 'Begin Part 3';
  const headsUpText = pillarIntro.headsUp;
  const roleTitle = pillarIntro.fingerprint?.roleTitle || '';
  const partNumber = pillarIntro.part ?? 3;
  const headerMiddle =
    pillarIntro.breadcrumb ||
    [roleTitle, `Part ${partNumber}`, 'Reasoning', levelLabel].filter(Boolean).join(' · ');
