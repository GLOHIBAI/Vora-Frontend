...
const RoleAssessmentStageTwoPartTwoExpertiseIntro: React.FC = () => {
  ...
  const {
    ...
    error,
    refetch,
  } = useStage2PillarIntroQuery(activeAssessmentId || '', 'expertise');
  ...
  const handleBegin = () => {
    ...
    navigate(`/onboarding/talent/${roleSlug}/interview/stage-2/part-2/interview-1`);
  };
      ...
      <div className="min-h-screen bg-[#F7F7F7] text-[#1A1A1A] font-sans flex items-center justify-center p-6">
        <div className="bg-white border border-[#E6E6E6] rounded-[18px] max-w-[440px] w-full p-[30px] text-center">
          <h2 className="text-[18px] font-[900] mb-2">
            {isOutOfSync ? 'Syncing Interview Progress' : 'Could not load Part 2 intro'}
          </h2>
          <p className="text-[14px] text-[#4A4A4A] leading-[1.6] mb-5">
            {isOutOfSync
              ? "Your interview is on a different section. Directing you to your active question..."
  ...
  const beginPartLabel = pillarIntro.ctas?.beginPartLabel || 'Begin Part 2';
  const headsUpText = pillarIntro.headsUp;
  const roleTitle = pillarIntro.fingerprint?.roleTitle || '';
  const headerMiddle =
    pillarIntro.breadcrumb ||
    [roleTitle, 'Part 2', 'Expertise', levelLabel].filter(Boolean).join(' · ');
