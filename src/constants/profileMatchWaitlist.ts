...
export const DEFAULT_PROFILE_WAITLIST_SUMMARY: ProfileWaitlistSummary = {
  ...
  strongProfileNote:
    "Your profile ranked in the top 15% of all profiles scanned this month",
...
export const MOCK_PROFILE_MATCH_SCAN_STRONG_NO_ROLES: ProfileMatchScanResult = {
  ...
  explanation: {
    summary:
      "Your profile is strong, but no live roles matched it at the current threshold.",
...
export const MOCK_PROFILE_MATCH_SCAN_ELIGIBILITY_ISSUE: ProfileMatchScanResult =
  {
    ...
    explanation: {
      decision: "INELIGIBLE",
      summary:
        "This role is open to candidates with existing right to work in the United Kingdom.",
  ...
  (threshold ??
    Math.round(
      (scan.scoreConfig?.matchThreshold ??
        DEFAULT_MATCH_SCORE_CONFIG.matchThreshold) * 100,
    ));
