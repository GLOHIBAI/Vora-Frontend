  ...
  return {
    version:
      typeof cfg.version === "number"
        ? cfg.version
        : DEFAULT_MATCH_SCORE_CONFIG.version,
    matchThreshold:
...
export const resolveMatchThresholdPercent = (
  ...
  const threshold =
    scan?.scoreConfig?.matchThreshold ??
    DEFAULT_MATCH_SCORE_CONFIG.matchThreshold;
...
export const resolveDimensionGapThresholdPercent = (
  ...
  const threshold =
    scan?.scoreConfig?.dimensionGapThreshold ??
    DEFAULT_MATCH_SCORE_CONFIG.dimensionGapThreshold;
  return Math.round(threshold * 100);
};
...
export const resolveProfileMatchScan = (
  ...
  ...MOCK_PROFILE_MATCH_SCAN,
  ...scan,
  scoreConfig: normalizeMatchScoreConfig(
    scan?.scoreConfig ?? MOCK_PROFILE_MATCH_SCAN.scoreConfig,
  ),
});
...
export const resolveMatchDisplayCopy = (
  ...
  switch (scan.outcome) {
    ...
    case "ELIGIBILITY_ISSUE":
      return {
        headline:
          resolveGateMessages(scan, false)[0] ??
          "You do not meet the eligibility requirements for this role.",
        body: "",
      };
    default:
      return {
...
export const hasAlternateRoleMatches = (
  scan: ProfileMatchScanResult,
): boolean => (scan.alternateMatches?.length ?? 0) > 0;
...
export const buildMatchConfirmedTitle = (
  ...
  const location =
    role.overviewRows.find((row) => row.label === "Location")?.value ??
    role.companyName;
...
export const buildMatchConfirmedSubtitle = (
  role: Pick<
    ...
    "compensationLine" | "metaItems" | "overviewRows"
  >,
): string => {
  const positionsRow = role.overviewRows.find(
    (row) => row.label === "Positions",
  )?.value;
  ...
  const salary =
    ...
    "$1,800/month";
  const expiry =
    role.metaItems.find((item) => item.includes("Expires")) ??
    "Expires in 28 days";
...
export const unwrapApiEnvelope = (
  response: unknown,
...
const normalizeMatchExplanation = (
  ...
  return {
    ...
    primaryReasonCode:
      (raw.primaryReasonCode as MatchExplanation["primaryReasonCode"]) ?? null,
    gates: Array.isArray(raw.gates)
      ? (raw.gates as MatchExplanation["gates"])
      : [],
    dimensionGaps,
    ...
    scoredDimensionCount:
      typeof raw.scoredDimensionCount === "number"
        ? raw.scoredDimensionCount
        : undefined,
    totalDimensionCount:
      typeof raw.totalDimensionCount === "number"
        ? raw.totalDimensionCount
        : undefined,
  };
};

const normalizeMatchDevelopment = (
  raw: unknown,
): MatchDevelopment | undefined => {
...
export const mapRolePostingSnapshot = (
  raw: unknown,
): MatchRolePostingSnapshot | undefined => {
  if (!raw || typeof raw !== "object") return undefined;
    ...
    companyName:
      typeof employer?.organisationName === "string"
        ...
        : typeof rp.companyName === "string"
          ...
          : "",
    workLocationCity:
      typeof rp.workLocationCity === "string" ? rp.workLocationCity : undefined,
    workLocationCountry:
      typeof rp.workLocationCountry === "string"
        ? rp.workLocationCountry
        : undefined,
...
export const resolveRoleLocationFromScan = (
  scan: ProfileMatchScanResult,
): string => {
...
export const mapApiMatchResultToScan = (
  ...
  const qualificationsScore: number = Number(
    (payload?.dimensionScores as Record<string, number> | undefined)
      ?.qualifications ?? rawScore,
  );
  ...
  const failedGeoGate =
    explanation?.gates?.some((gate) => !gate.passed) ?? false;
  const isEligible =
  ...
  return {
    ...
    development,
    rolePosting,
    dimensionScores: payload?.dimensionScores as
      | Record<string, number>
      | undefined,
    rolePostingId:
      typeof payload?.rolePostingId === "string"
        ? payload.rolePostingId
        : typeof (payload?.rolePosting as Record<string, unknown> | undefined)
              ?.id === "string"
...
export const isMatchResultPending = (apiResponse: unknown): boolean => {
  ...
  const data =
    unwrapApiEnvelope(apiResponse) ?? (apiResponse as Record<string, unknown>);
