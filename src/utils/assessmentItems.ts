...
export const normalizeAssessmentItemType = (type: string): AssessmentItemType => {
...
export const normalizeAssessmentItem = (raw: unknown): AssessmentItem | null => {
...
export const normalizeAssessmentItems = (items: unknown): AssessmentItem[] =>
  Array.isArray(items)
    ? items
        ...
        .filter((item): item is AssessmentItem => item !== null && Boolean(item.id))
    : [];
...
const normalizeProgress = (raw: unknown): AssessmentProgress => {
  ...
  if (typeof progress.percent === 'number') {
    return {
      ...
      answered: typeof progress.answered === 'number' ? progress.answered : undefined,
...
export const normalizeGateStartResponse = (
  ...
  return {
    ...
    screenKey: String(payload.screenKey ?? payload.screen_key ?? screenKey ?? ''),
          ...
          limitSeconds:
            typeof timersRaw.limitSeconds === 'number'
              ...
              : typeof timersRaw.gateLimitSecs === 'number'
                ...
                : undefined,
          warnAtSeconds:
            typeof timersRaw.warnAtSeconds === 'number' ? timersRaw.warnAtSeconds : undefined,
          startedAt: typeof timersRaw.startedAt === 'string' ? timersRaw.startedAt : undefined,
        }
      : undefined,
    sessionState:
...
const normalizeAdaptiveStepContent = (
  ...
  return {
    ...
    prompt: typeof (raw.prompt ?? stem) === 'string' ? String(raw.prompt ?? stem) : undefined,
    options: Array.isArray(options) ? options : undefined,
  };
};

/** POST .../items/:itemId/adaptive — unwrap envelope; map stem + options for next step. */
export const normalizeAdaptiveStepResponse = (raw: unknown): AdaptiveStepResponse => {
  const payload = unwrapAssessmentData<Record<string, unknown>>(raw) ?? {};

  let nextItem = normalizeAdaptiveStepContent(
    (payload.nextItem ?? payload.next_item) as Record<string, unknown> | undefined,
  );
  ...
  return {
    ...
    nextItem,
    componentStatus:
      payload.componentStatus === 'completed' || payload.component_status === 'completed'
        ? 'completed'
        : undefined,
  };
};
...
export const normalizeSaveDraftResponse = (raw: unknown): SaveDraftResponse => {
  const payload = unwrapAssessmentData<Record<string, unknown>>(raw) ?? (raw as Record<string, unknown>);
  return {
    items: normalizeAssessmentItems(payload?.items),
    questionsRegenerated: payload?.questionsRegenerated === true,
...
export const getForcedChoiceBlocks = (item: AssessmentItem): ForcedChoiceBlock[] => {
  const rawBlocks = item.content.blocks as Array<Record<string, unknown>> | undefined;
      ...
      const statements = Array.isArray(statementsRaw)
        ? statementsRaw.map((row, stmtIdx) => {
            if (!row || typeof row !== 'object') {
              return { id: `${block.id ?? index}-s${stmtIdx}`, label: String(row) };
            }
...
export const getValuesAbPairs = (item: AssessmentItem): ValuesAbPair[] => {
  const rawPairs = item.content.pairs as Array<Record<string, unknown>> | undefined;
  ...
  return rawPairs.map((pair, index) => {
    ...
    return {
      id,
      labelA: String(a?.label ?? pair.labelA ?? 'Option A'),
      labelB: String(b?.label ?? pair.labelB ?? 'Option B'),
    };
  });
};
...
export const getValuesTradeoffTensions = (item: AssessmentItem): ValuesTradeoffTension[] => {
  const raw =
  ...
  return raw.map((row, index) => {
    ...
    return {
      ...
      leftLabel: String(left?.title ?? left?.label ?? a?.label ?? row.leftLabel ?? 'A'),
      rightLabel: String(right?.title ?? right?.label ?? b?.label ?? row.rightLabel ?? 'B'),
