...
export const parseGateResumeState = (
  response: unknown,
  ...
  return {
    ...
    assessmentId:
      typeof obj.assessmentId === "string" ? obj.assessmentId : undefined,
    session,
    sessionLabel: String(
      obj.sessionLabel ??
        obj.session_label ??
        (session === 1 ? "How you think" : "Your instincts"),
    ),
    nextScreenKey: nextScreenKey as Gate1ScreenKey,
    inProgress,
    completedScreenKeys,
    session1Screens: Array.isArray(obj.session1Screens)
      ? (obj.session1Screens as any[])
      : [...GATE1_SESSION1_SCREENS],
    session2Screens: Array.isArray(obj.session2Screens)
      ? (obj.session2Screens as any[])
      : [...GATE1_SESSION2_SCREENS],
    ...
    gateDeadlineTotalSeconds:
      typeof obj.gateDeadlineTotalSeconds === "number"
        ? obj.gateDeadlineTotalSeconds
        : undefined,
    screenTimerLimitSeconds:
      typeof obj.screenTimerLimitSeconds === "number"
        ? obj.screenTimerLimitSeconds
        : undefined,
  };
};

/** Normalize gates/progress API payloads array, `{ gates: [] }`, or single entry. */
export const parseGateProgressEntries = (
  response: unknown,
): GateProgressEntry[] => {
  const unwrapped = unwrapAssessmentData<unknown>(response);
  ...
  if (unwrapped && typeof unwrapped === "object") {
    ...
    if (typeof obj.gate === "number") {
      return [obj as unknown as GateProgressEntry];
    }
  }
  return [];
};

export const findGate1ProgressEntry = (
  response: unknown,
): GateProgressEntry | null =>
...
const isReviewSummaryEntry = (value: unknown): value is ReviewSummaryEntry => {
  ...
  return (
    typeof entry.screenKey === "string" && typeof entry.componentId === "string"
  );
};

/** Normalize review-summary API payloads array or nested `{ entries | screens | items }`. */
export const parseReviewSummaryEntries = (
  response: unknown,
): ReviewSummaryEntry[] => {
  const unwrapped = unwrapAssessmentData<unknown>(response);
  if (Array.isArray(unwrapped)) {
...
export const resolveGate1StartScreenKey = (
  resume: GateResumeState,
): Gate1ScreenKey => resume.inProgress?.screenKey ?? resume.nextScreenKey;
...
export const getGate1HighLevelTags = (resume: GateResumeState): string[] => {
  const tags: string[] = [];
  if (resume.session1Screens.includes("personality"))
    tags.push(GATE1_SCREEN_LABELS.personality);
  if (resume.session1Screens.includes("values"))
    tags.push(GATE1_SCREEN_LABELS.values);
  if (resume.session1Screens.some((k) => GATE1_COGNITIVE_KEYS.has(k)))
    tags.push("Cognitive");
...
export const resolveGate1ActiveScreenKey = (
  resume: GateResumeState,
): Gate1ScreenKey => resume.inProgress?.screenKey ?? resume.nextScreenKey;
